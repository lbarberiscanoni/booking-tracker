            ...
            if (todaysDate <= guestStartDate) {
                if (guestStartDate <= threeDaysFromNowFormatted) {
                    if (guestStartDate.toString().split(" ", 4).join(" ") == todaysDate.toString().split(" ", 4).join(" ")) {
                        $("#checkIns").append("<h3 class='btn btn-default' style='background-color: red; color: white;'>" + guest.title + "<br>" + guestStartDateFormatted + "<br>[" + guest.location + "]</h3>");
                    } else {
                        $("#checkIns").append("<h3 class='btn btn-default' style='background-color: green; color: white;'>" + guest.title + "<br>" + guestStartDateFormatted + "<br>[" + guest.location + "]</h3>");
                    };
                };
            } else {
            ...
            if (todaysDate <= guestEndDate) {
                if (guestEndDate <= threeDaysFromNowFormatted) {
                    if (guestEndDate.toString().split(" ", 4).join(" ") == todaysDate.toString().split(" ", 4).join(" ")) {
                        $("#checkOuts").append("<h3 class='btn btn-default' style='background-color: red; color: white;'>" + guest.title + "<br>" + guestEndDateFormatted + "<br>[" + guest.location + "]</h3>");
                    } else {
                        $("#checkOuts").append("<h3 class='btn btn-default' style='background-color: green; color: white;'>" + guest.title + "<br>" + guestEndDateFormatted + "<br>[" + guest.location + "]</h3>");
                    };
                };
            } else {
                console.log("not in the date range");
